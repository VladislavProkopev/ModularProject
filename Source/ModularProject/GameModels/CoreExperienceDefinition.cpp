...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreExperienceDefinition)
...
#if WITH_EDITOR
EDataValidationResult UCoreExperienceDefinition::IsDataValid(class FDataValidationContext& Context) const
{
	EDataValidationResult Result = CombineDataValidationResults(Super::IsDataValid(Context),EDataValidationResult::Invalid);
	...
	if (!GetClass()->IsNative())
	{
		...
		{
			Context.AddError(FText::Format(LOCTEXT("ExperienceInheritenceIsUnsupported","Blueprint sublclass of Blueprint experiences is not currently supported (uce composition via ActionSets instead). Parent class was {0} but should be {1}"),
...
void UCoreExperienceDefinition::UpdateAssetBundleData()
{
