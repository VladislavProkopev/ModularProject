...
#include "ModularProject/Character/ProjectHeroComponent.h"
...
{
	RegisterInputContextMappingsForGameInstanceHandle = FWorldDelegates::OnStartGameInstance.AddUObject(this,&UGameFeatureAction_AddInputContextMapping::RegisterInputMappingsForGameInstance);
	...
	{
		RegisterInputMappingsForGameInstance(WorldContextIterator->OwningGameInstance);
	}
}
	...
	if (ensure(LocalPlayer))
	{
		if (UEnhancedInputLocalPlayerSubsystem* EISubsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
		{
			if (UEnhancedInputUserSettings* Settings = EISubsystem->GetUserSettings())
			{
				for (const FInputMappingContextAndPriority& Entry : InputMappings)
				{
					//Skip entries that don't want to be redistered
					if (!Entry.bRegisterWithSettings)
					{
						continue;
					}
	...
	FGameFeatureStateChangeContext ChangeContext)
{
	...
	else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == UProjectHeroComponent::NAME_BindInputsNow))
	{
		AddInputMappingForPlayer(AsController->GetLocalPlayer(), ActiveData);
       
		// FIX: Регистрируем контроллер, чтобы Reset() мог корректно зачистить инпуты 
		// при runtime-выгрузке GameFeature.
		ActiveData.ControllersAddedTo.AddUnique(AsController);
	}
}
