...
bool ILoadingProcessInterface::ShouldShowLoadingScreen(UObject* TestObject,FString& OutReason)
{
	if (TestObject != nullptr)
	{
		if (ILoadingProcessInterface* LoadObserver = Cast<ILoadingProcessInterface>(TestObject))
		{
			...
			{
				if (ensureMsgf(!ObserverReason.IsEmpty(),TEXT("%s is failed to set a reason why it wants to show the loading screen"),*GetPathNameSafe(TestObject)))
				{
...
{
	// CVars
	static float HoldLoadingScreenAdditionalSecs = 2.f;
	static FAutoConsoleVariableRef CVarHoldLoadingScreenUpAtLeastThisLongInSecs(
		...
		TEXT("How long to hold loading screen up after other loading finishes (in seconds) to try give texture streaming a chance to avoid blurriness"),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarForceLoadingScreenVisible(
		...
		TEXT("Force the loading to show."),
		ECVF_Default);
}
...
//Input processor to throw when loading screen is shown
...
void UCoreLoadingScreenManager::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UCoreLoadingScreenManager::Deinitialize()
{
	StopBlockingInput();
...
bool UCoreLoadingScreenManager::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UCoreLoadingScreenManager::Tick(float DeltaTime)
{
	UpdateLoadingScreen();
...
ETickableTickType UCoreLoadingScreenManager::GetTickableTickType() const
{
	if (IsTemplate())
	{
...
bool UCoreLoadingScreenManager::IsTickable() const
{
...
TStatId UCoreLoadingScreenManager::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UCoreLoadingScreenManager, STATGROUP_Tickables);
}

UWorld* UCoreLoadingScreenManager::GetTickableGameObjectWorld() const
{
	return GetGameInstance()->GetWorld();
}

void UCoreLoadingScreenManager::RegisterLoadingProcessor(TScriptInterface<ILoadingProcessInterface> Interface)
{
	ExternalLoadingProcessors.Add(Interface.GetObject());
}

void UCoreLoadingScreenManager::UnregisterLoadingProcessor(TScriptInterface<ILoadingProcessInterface> Interface)
{
	ExternalLoadingProcessors.Remove(Interface.GetObject());
}

void UCoreLoadingScreenManager::HandlePreloadMap(const FWorldContext& WorldContext, const FString& MapName)
{
...
void UCoreLoadingScreenManager::HandlePostLoadMap(UWorld* World)
{
...
void UCoreLoadingScreenManager::UpdateLoadingScreen()
{
	...
	if (ShouldShowLoadingScreen())
	{
		...
		if ((Settings->LogLoadingScreenHeartbeatInterval > 0.f) && (TimeUntilNextLogHeartbeatSeconds <= 0.f))
		{
...
bool UCoreLoadingScreenManager::CheckForAnyNeedToShowLoadingScreen()
{
	...
	if (bCurrentlyInLoadMap)
	{
		...
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("bCurrentlyInLoadMap = true"));
		return true;
	}
	...
	{
		//Show a loading screen during steamless travel
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("We are in steamless travel"));
		return true;
	}
	...
	//that were registered by game core to tell us to keep the loading screen up while perhaps something finishes
	//streaming in.
	...
	{
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("At least one missing local player controller in split screen"));
		return true;
	}
	
	//And non-splitscreen we need at least one player controller to be present
	...
	DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("(nothing wants to show it anymore)"));
	return false;
}
...
bool UCoreLoadingScreenManager::ShouldShowLoadingScreen()
{
	...
	{
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("Command line has 'NoLoadingScreen' option'"));
		return false;
	}
#endif
	...
	if (bNeedToShowLoadingScreen)
	{
		...
		TimeLoadingScreenLastDismissed = -1.f;
	}
	else
	{
		...
		{
			//Make sure we're rendering the world at this point, so that textures will actually steam in
...
bool UCoreLoadingScreenManager::IsShowingInitialLoadingScreen() const
{
...
void UCoreLoadingScreenManager::ShowLoadingScreen()
{
	...
	//Unable to show loading screen if the engine is loading with its loading screen.
	...
	else
	{
		...
		//Eat input while loading screen is displayed
		StartBlockingInput();
		...
		//Add to the viewport at high ZOrder to make sure it is on top of most things
		UGameViewportClient* GameViewportClient = LocalGameInstance->GetGameViewportClient();
		GameViewportClient->AddViewportWidgetContent(LoadingScreenWidget.ToSharedRef(),Settings->LoadingScreenZOrder);
		
		ChangePerformSettings(/** bEnableLoadingScreen */ true);
...
void UCoreLoadingScreenManager::HideLoadingScreen()
{
	if (!bCurrentlyShowingLoadingScreen)
	{
		return;
...
void UCoreLoadingScreenManager::RemoveWidgetFromViewport()
{
	UGameInstance* GameInstance = GetGameInstance();
...
void UCoreLoadingScreenManager::StartBlockingInput()
{
...
void UCoreLoadingScreenManager::StopBlockingInput()
{
	if (InputPreProcessor.IsValid())
	{
...
void UCoreLoadingScreenManager::ChangePerformSettings(bool bEnablingLoadingScreen)
{
	...
	{
		//Restore the hang detector timeout when we hide loading screen
